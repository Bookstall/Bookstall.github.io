...
例如：Flat、k-means、LSH 等等。相似最近邻搜索问题是向量数据库产品的核心，一个成熟的向量数据库通常集成了多种ANN算法，并在搜索时综合各种指标选择最合适的策略来利用这些算法。
...
LangChain，并以 **ChromeDB** 为例，演示了如何用 LangChain 对接向量数据，做一个本地的文档知识库。
